...
struct Coordinates {
    ...
    Coordinates(I the_i, I the_j) : i(the_i), j(the_j) {}

    Coordinates(const std::tuple<I, I>& t)
    ...
    Coordinates& operator=(const std::tuple<I, I>& t) {
        i = std::get<0>(t);
        j = std::get<1>(t);
        return *this;
    }
    ...
    Coordinates operator*(I k) const {
        return {i * k, j * k};
    }

    friend Coordinates operator*(I k, const Coordinates& c) {
        return c * k;
    }
    ...
    Coordinates& operator*=(I k) {
        return *this = *this * k;
    }

    std::strong_ordering operator<=>(const Coordinates&) const = default;
    ...
    Coordinates RotateLeft() const {
        return {-j, i};
    }

    // Rotates 180 degees.
    Coordinates Flip() const {
        return {-i, -j};
    }
...
template <typename I>
struct std::hash<Coordinates<I>> {
    size_t operator()(const Coordinates<I>& c) const {
        return SeqHash(c.i, c.j);
    }
};

using Coord = Coordinates<int>;
