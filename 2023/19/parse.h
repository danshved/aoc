...
// The version of Split2 for strings, so that one can use a string literal here:
//
// std::string s = "a b";
// Split2(s, " ");
std::tuple<std::string, std::string> Split2(const std::string& s, const std::string& delim) {
    return SplitN(s, delim);
}
