...
// Chains comparison results lexicographically.
...
std::strong_ordering Lex(std::partial_ordering x, Ts... ys) {
    return (x == std::partial_ordering::less) ? std::strong_ordering::less :
        (x == std::partial_ordering::greater) ? std::strong_ordering::greater :
        Lex(ys...);
}

int CharToCard(unsigned char ch) {
    return (ch >= '2' && ch <= '9') ? (ch - '0') :
        (ch == 'T') ? 10 : (ch == 'J') ? 11 : (ch == 'Q') ? 12 : (ch == 'K') ? 13 : (ch == 'A') ? 14 : 0;
}
...
struct Hand {
    ...
    std::weak_ordering operator<=>(const Hand& other) const {
        return Lex(groups <=> other.groups, cards <=> other.cards);
    }
};
