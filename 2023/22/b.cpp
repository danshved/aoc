...
int main() {
    ...
    std::unordered_map<Brick, std::vector<Brick>> children;
    std::unordered_map<Brick, int> parent_count;
    for (const Brick& u : bricks) {
        for (const Brick& v : bricks) {
    ...
    int answer = 0;
    for (const Brick& s : bricks) {
        std::unordered_map<Brick, int> support_count = parent_count;
        auto visit = [&](auto& self, const Brick& u) -> void {
            for (const Brick& v : children[u]) {
                support_count[v]--;
                if (support_count[v] == 0) {
                    answer++;
                    self(self, v);
                }
            }
